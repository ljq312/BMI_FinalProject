...
ghc_cate <- ghc %>% 
  select(c(5, 7, 8, 10, 13:18 )) ## removed admitdtm and postalcode from frequency table

map(ghc_cate, plyr::count) 
...
ghc_final <- ghc_final %>% 
                ...
                dplyr::select(-patientid, -visitid, -postalcode, 
                       -facilityzip, -date, -losdays2, -icu_flag, -group_mews, -cindex)
...
MainE_Model <- lm(formula = log_losday ~ . - Month, 
    data = ghc_final)
